...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.edit("english voice recognition is initiated.....")
...
@register(outgoing=True, pattern=r"^\.stthindi(?: |$)(.*)")
async def _(event):
    if not event.reply_to_msg_id:
        return await event.edit("Reply to any user's media message.")
    reply_message = await event.get_reply_message()
    ...
    async with event.client.conversation(chat) as conv:
        ...
        await event.edit(text)
        await event.client.delete_messages(
            conv.chat_id, [msg_start.id, response.id, result.id, speech.id, langsettohindi.id, thumpsup.id]
        )
...
CMD_HELP.update(
    {
        "stt": ".stt"
                 ...
                 "using `@voicybot`"
                 "\n\n stthindi"
                 "\nusage: Convert a hindi voice speech hindi text"
    }
)
