...
async def download(gdrive, service, uri=None):
    ...
    if uri:
        ...
        try:
            ...
            reply += (
                "`[FILE - CANCELLED]`\n\n"
                "`Status` : **DONE** - received signal cancelled."
            )
            return reply
        else:
        ...
        if isfile(required_file_name):
            try:
                ...
                reply += (
                    ...
                    "`Status :` **DONE** - Successfully uploaded.\n\n"
                )
                return reply
        else:
...
async def download_gdrive(gdrive, service, uri):
    ...
    await gdrive.edit(
        ...
        f"`Path   :` `{file_path}`\n"
        "`Status :` **DONE** - Successfully downloaded."
    )
...
@register(pattern=r"^.list(?: |$)(-l \d+)?(?: |$)?(.*)?(?: |$)", outgoing=True)
    ...
    while True:
        ...
        for files in response.get("files", []):
            ...
            if files.get("mimeType") == "application/vnd.google-apps.folder":
                ...
                message += f"`Size   :` `{humanbytes(result[0])}`\n"
            else:
...
async def google_drive(gdrive):
    ...
    if result:
        await gdrive.respond(
            ...
            f"`Size   :` `{humanbytes(result[0])}`\n"
            f"`Link   :` [{file_name}]({result[1]})\n"
            "`Status :` **DONE** - Successfully uploaded to google drive.\n",
            link_preview=False,
        )
    await gdrive.delete()
    return
...
async def set_upload_folder(gdrive):
    ...
    if exe == "rm":
        if G_DRIVE_FOLDER_ID is not None:
            ...
            await gdrive.edit(
                "`[FOLDER - SET]`\n\n"
                "`Status` : **DONE** - using `G_DRIVE_FOLDER_ID` now."
            )
            return None
        else:
            try:
    ...
    try:
        ...
        if True in [c1 or c2]:
            parent_Id = inp
            await gdrive.edit(
                "`[PARENT - FOLDER]`\n\n" "`Status` : **DONE** - Successfully changed."
            )
            return None
        else:
            await gdrive.edit(
