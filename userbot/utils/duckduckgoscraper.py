...
class DuckDuckGoScraper:
        ...
        if self.proxies:
            for _ in range(len(self.proxies) + 1):
                try:
                    ...
                    warnings.warn(
                        'Encountered Payload error. This is caused due to unacceptable headers. Continuing'
                    )
    ...
    async def _get_json(self, session, query, page, vqd):
        search_url = 'https://duckduckgo.com/i.js?' + urllib.parse.urlencode(
            {
                'q': query,
                'o': 'json',
                'p': page + 1,
    ...
    def scrape(self, query, num_pages, out_path=None, proxies=None):
        ...
        loop2.create_task(self._main_exec(query, num_pages, out_path))
